...
このプロジェクトでは ISR を使っているページ（`/` と `/lives`）で「データは正しいのに画面に反映されない」バグが頻発している。デバッグ時は**最初に ISR キャッシュを疑うこと**。

### ISR ページでデータが反映されない場合のチェックリスト

1. **対象ページのレンダリング戦略を確認する**
   - `/`・`/lives` は ISR → ビルド時点のデータをキャッシュ
   - `/members`（CSR）・`/admin`（SSR）は毎回新鮮なデータを取得
   - 「CSR/SSRでは見える・ISRページでは見えない」なら ISR キャッシュ確定

2. **ISR キャッシュを無効化する方法**
   - `revalidatePath()` が正しく呼ばれているか確認する（`createPhotoAction`、`createLive` 等のServer Action）
   - 外部設定（Supabase 設定変更など）を変えた後は、空コミットで再デプロイして ISR を作り直す
     ```bash
     git commit --allow-empty -m "chore: ISRキャッシュ再構築のため再デプロイ"
     git push origin master
     ```

3. **新機能追加・DB設定変更後は必ず再デプロイを確認する**
   - Supabase の `db-max-rows` 等の外部設定変更は、変更後に再デプロイしないと ISR に反映されない
   - コード変更と外部設定変更が同タイミングのときは、外部設定変更 → 再デプロイの順で行う
