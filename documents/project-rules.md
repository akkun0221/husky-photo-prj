# project-rules.md — ちくわフォトアルバム プロジェクト固有ルール

このファイルは `CLAUDE.md` のプロジェクト固有ルールです。
`CLAUDE.md` の汎用ルールと必ず合わせて参照すること。
機能の詳細仕様は `RDD.md` を参照すること。
...
| 状態管理（クライアント） | Zustand（Zustand Context Pattern必須） |
| 状態管理（サーバー）     | TanStack Query v5                      |
...
photos テーブル
  ...
  r2_url (text)         ← フルサイズ原版。ライトボックス（拡大表示）でのみ使用
  thumbnail_url (text)  ← 表示用サムネイル（長辺300px・WebP）。グリッド・スライドショー等すべての一覧表示で使用
```
...
- **`design/` 配下にリファレンスファイルが存在する場合**：実装対象のページ・コンポーネントとリファレンスを1対1で照合し、差分がないことを確認してからビルドチェックに進む
...
4. `design/` 配下にリファレンスがある場合は照合してから、`tsc --noEmit` と `next build` でエラーがないことを確認する
